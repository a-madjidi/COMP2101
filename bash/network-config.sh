...
HOST=$HOSTNAME
LAN_IP=$(ip route get 8.8.8.8 | sed -n '/src/{s/.*src *\([^ ]*\).*/\1/p;q}')
LANHOST=$(getent hosts $LAN_IP | awk '{print $2}')
EXT_IP=$(curl -s icanhazip.com)
EXTNAME=$(getent hosts $EXT_IP | awk '{print $2}')
...
DEF_IP=$(ip route get 8.8.8.8 | sed -n '/via/{s/.*via *\([^ ]*\).*/\1/p;q}')
DEFNAME=$(getent hosts $DEF_IP | awk '{print $2}')
