...
[ $# -gt 2 ] && echo "Invalid number of arguments"
[[ $# -eq 1 && $1 == "-v" ]] || [[ $# -eq 2 && $2 == "-v" ]] && verbose=$"yes"
[[ $# -eq 1 && $1 != "-v" ]] || [ $# -eq 2 ] && interface=$1
...
# test to see if interface argument exists in interfaces list
if [ ! -z $interface ]; then
	found=false
	for h in "${interfaces[@]}"; do
    	if [[ $interface = "$h" ]]; then
        	found=true
        	break
    	fi
	...
	if [[ ! $found == true ]]; then
    	echo "Inputted interface '$interface' not found"
	fi
fi

# loop through the interfaces list
for i in "${interfaces[@]}"; do
	if [ -z $interface ] || [[ $i == $interface ]]; then
		...
		# networknumber  e.g. grep -q mynetworknumber /etc/networks || (echo 'mynetworkname mynetworknumber' |sudo tee -a /etc/networks)
